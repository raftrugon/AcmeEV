...
class EnrollmentSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $students = User::join('model_has_permissions','users.id','=','model_has_permissions.model_id')->where('model_has_permissions.permission_id', 2)->whereNotNull('degree_id')->get()->toArray();
        ...
        foreach ($students as $student){
            ...
            $starting_academic_year = $faker->randomBetween($min_academic_year, $max_academic_year);
            ...
            for($i = $starting_academic_year; $i <= $actual_academic_year; $i++){
                ...
                $not_passed_subjects = SubjectInstance::where('academic_year', $i)                                      //Busca las subject_instances con año académico actual
                    ...
                    ->get()->pluck('id')->toArray();                                                                    //Extrae las asignaturas
                ...
                for($i = 0; $i <= $number_of_enrollments_this_year; $i++){
                    $subject_instance_id = $faker->randomElement($not_passed_subjects);
                    ...
                    //Asignación a grupos
                    $group = $faker->randomElement(SubjectInstance::where('id',$subject_instance_id)->get->getGroups());
